...
void cribbage::setup_play_phase() {
    cards_played[8];
    cards_played_since_new_stack[8];
    num_cards_played = 0;
...
int cribbage::play_phase() {
    ...
    while (pone->get_hand()->get_num_cards() || dealer->get_hand()->get_num_cards()) {
        ...
        if (exsists_legal_move(current_player->get_hand()->get_cards(), current_player->get_hand()->get_num_cards(), sum_cards)) {
            acting_player_action = current_player->poll_player(false, cards_played_since_new_stack, num_cards_played_since_new_stack, sum_cards, current_opp->get_hand()->get_num_cards(), *current_player_score, *current_opp_score, !pone_to_play);
            //Checking is move is legal
            if(!check_valid_move(false, cards_played, num_cards_played, sum_cards, crib.get_cards(), crib.get_num_cards(), current_player->get_hand()->get_cards(), current_player->get_hand()->get_num_cards(), acting_player_action)) {
...
int cribbage::handle_discards() {
    ...
    if(!check_valid_move(true, nullptr, 0, 0, nullptr, 0, dealer->get_hand()->get_cards(), dealer->get_hand()->get_num_cards(), dealer_discard)) {
        cout << "Player 1 tried to do an illigal move" << endl;
        return -1;
    }
    if(!check_valid_move(true, nullptr, 0, 0, nullptr, 0, pone->get_hand()->get_cards(), pone->get_hand()->get_num_cards(), pone_discards)) {
...
int cribbage::round() {
    ...
    pone_discards = pone->poll_player(true, 0, 0, 0, 6, *pone_score, *dealer_score, false);
    dealer_discard = dealer->poll_player(true, 0, 0, 0, 6, *dealer_score, *pone_score, true);
